            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <span class="text-xs font-semibold px-2.5 py-1 rounded-full bg-slate-100 text-slate-600">
                        {{ $leaves->total() ?? $leaves->count() }} Records
                    </span>
                </div>
                            ...
                            @forelse ($leaves as $leave)
                                <tr class="hover:bg-slate-50/60 transition-colors">
                                    ...
                                    <td class="px-6 py-3.5">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold border {{ $pillColor }}">
                                    ...
                                    <td class="px-5 py-3.5 text-right font-sans">
                                        @if ($leave->status === 'pending')
                                            <form action="{{ route('staff.leaves.destroy', $leave) }}" method="POST"
