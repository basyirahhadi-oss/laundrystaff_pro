...
<head>
    ...
    <title>Payslip_{{ $payroll->full_name }}_{{ $payroll->month_year }}</title>
    <style>
...
<body>
    ...
    <div class="payslip-box">
        <div class="header">
            <img src="{{ asset('images/zaujati-logo.png') }}" alt="Zaujati Laundry" style="width: 72px; height: 72px; object-fit: contain; margin-bottom: 8px; background: #0f172a; padding: 4px; border-radius: 16px;">
        ...
        <table class="meta-table">
            <tr>
                ...
                <td class="meta-value uppercase">{{ $payroll->full_name }}</td>
                <td class="meta-label">Statement For:</td>
                <td class="meta-value accent">{{ $payroll->month_year }}</td>
            </tr>
            <tr>
                <td class="meta-label">Staff ID:</td>
                <td class="meta-value mono">{{ $payroll->staff_id }}</td>
                <td class="meta-label">Date Generated:</td>
                <td class="meta-value">{{ date('d M Y', strtotime($payroll->created_at)) }}</td>
            </tr>
            <tr>
                <td class="meta-label">Position:</td>
                <td class="meta-value">{{ $payroll->position }}</td>
                <td class="meta-label">Contact No:</td>
                <td class="meta-value">{{ $payroll->phone_number }}</td>
            </tr>
        </table>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    <td>Overtime Allowance <span style="font-size: 12px; color: #9ca3af;">({{ $payroll->ot_hours }} Hours)</span></td>
                    <td class="text-right text-success">+{{ number_format($payroll->ot_pay, 2) }}</td>
                    <td class="text-right">&mdash;</td>
                </tr>
                <tr>
                    <td>EPF / KWSP Contribution (11%)</td>
                    <td class="text-right">&mdash;</td>
                    <td class="text-right text-danger">{{ number_format($payroll->epf_deduction, 2) }}</td>
                </tr>
                <tr>
                    <td>SOCSO / PERKESO Contribution</td>
                    <td class="text-right">&mdash;</td>
                    <td class="text-right text-danger">{{ number_format($payroll->socso_deduction, 2) }}</td>
                </tr>
                <tr>
                    <td>EIS / SIP Contribution (0.2%)</td>
                    <td class="text-right">&mdash;</td>
                    <td class="text-right text-danger">{{ number_format($payroll->eis_deduction, 2) }}</td>
                </tr>
                <tr class="total-row">
                    <td>Net Salary (Payout)</td>
                    <td colspan="2" class="text-right" style="font-size: 18px;">RM {{ number_format($payroll->net_salary, 2) }}</td>
                </tr>
            </tbody>
        </table>
