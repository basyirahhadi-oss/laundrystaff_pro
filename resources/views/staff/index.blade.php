                    ...
                    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
                        <div class="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                            ...
                            <span class="text-xs font-bold px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 font-mono-nums">
                                {{ $staffList->count() }} Employees Enrolled
                            </span>
                        </div>
                                        ...
                                        <tr class="hover:bg-slate-50/70 transition-colors group">
                                            ...
                                            <td class="px-5 py-3.5">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    @endif
                                                    <div>
                                                        <div class="font-bold text-slate-900 text-xs">{{ $staff->full_name }}</div>
                                            ...
                                            <td class="px-5 py-3.5 font-bold font-mono-nums text-slate-900">
                                                RM {{ number_format($staff->salary_rate ?? $staff->salary ?? 0, 2) }}
                                            ...
                                            <td class="px-5 py-3.5 text-right">
                                                <div class="flex items-center justify-end gap-1.5">
                                                    ...
                                                    @if(auth()->user()->role === 'admin' || auth()->user()->email == 'admin@zaujati.com')
                                                        <a href="{{ route('staff.payroll.create', ['id' => $staff->staff_id]) }}"
                                                        ...
                                                        <form action="{{ route('staff.destroy', $staff->staff_id) }}" method="POST" class="inline-block"
                                                              onsubmit="return confirm('Confirm deletion of {{ addslashes($staff->full_name) }}?');">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
