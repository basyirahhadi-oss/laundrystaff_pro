            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-xs">
                <form method="GET" class="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-end gap-3.5">
                    ...
                    <div class="flex-1 min-w-[220px]">
                        ...
                        <select name="staff_id"
                            ...
                            <?php $__currentLoopData = $staffList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $staff): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($staff->id); ?>" <?php echo e((string) $staffId === (string) $staff->id ? 'selected' : ''); ?>>
                                    <?php echo e($staff->name); ?> (<?php echo e(ucfirst($staff->role)); ?>)
                                </option>
                    ...
                    <div class="flex items-center gap-2 pt-1 sm:pt-0">
                        ...
                        <?php if($staffId || $month !== now()->format('Y-m')): ?>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Records for <?php echo e(\Carbon\Carbon::createFromFormat('Y-m', $month)->format('F Y')); ?></p>
                    </div>
                    <span class="text-xs font-medium px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 font-mono-nums">
                        Showing <?php echo e($attendances->count()); ?> of <?php echo e($attendances->total()); ?> records
                    </span>
                </div>
                            ...
                            <?php $__empty_1 = true; $__currentLoopData = $attendances; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr class="hover:bg-slate-50/60 transition-colors group">
                                    <td class="px-6 py-4 font-medium text-slate-900">
                                        <div class="flex items-center gap-3">
                                            <div class="w-8 h-8 rounded-lg bg-slate-100 text-slate-700 flex items-center justify-center font-bold text-xs flex-shrink-0">
                                                <?php echo e(substr($row->staff->name ?? 'U', 0, 2)); ?>

                                            </div>
                                            <div>
                                                <div class="font-semibold text-slate-900"><?php echo e($row->staff->name ?? 'Unlinked Staff'); ?></div>
                                                <div class="text-[11px] text-slate-400"><?php echo e($row->staff->email ?? '—'); ?></div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 font-mono-nums text-slate-700">
                                        <div class="font-medium text-slate-900"><?php echo e($row->date->format('d M Y')); ?></div>
                                        <div class="text-[11px] text-slate-400"><?php echo e($row->date->format('l')); ?></div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        ?>
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold border <?php echo e($badgeClass); ?>">
                                            <?php echo e($label); ?>
                                    ...
                                    <td class="px-6 py-4 text-right whitespace-nowrap">
                                        <div class="inline-flex items-center gap-2 justify-end">
                                            <button type="button"
                                                onclick="document.getElementById('edit-<?php echo e($row->id); ?>').classList.toggle('hidden')"
                                            ...
                                            <form action="<?php echo e(route('admin.attendance.destroy', $row->id)); ?>" method="POST"
                                                  onsubmit="return confirm('Delete attendance entry for <?php echo e(addslashes($row->staff->name ?? 'this staff')); ?> on <?php echo e($row->date->format('d M Y')); ?>? This action is audited.');"
                                                  class="inline-block">
                                                <?php echo csrf_field(); ?>
                                ...
                                <tr id="edit-<?php echo e($row->id); ?>" class="hidden bg-slate-50/70 border-y border-slate-200">
                                    <td colspan="7" class="px-6 py-4">
                                        <form action="<?php echo e(route('admin.attendance.update', $row)); ?>" method="POST"
                                            ...
                                            <div>
                                                ...
                                                <input type="time" name="clock_in_time"
                                                    value="<?php echo e($row->clock_in_time?->format('H:i')); ?>"
                                            ...
                                            <div>
                                                ...
                                                <input type="time" name="clock_out_time"
                                                    value="<?php echo e($row->clock_out_time?->format('H:i')); ?>"
                                                    class="border border-slate-200 rounded-lg px-3 py-1.5 text-xs text-slate-800 font-mono-nums">
                                            </div>
                                            <div>
                                                <label class="block text-xs font-semibold text-slate-500 mb-1">Status</label>
                                                <select name="status" class="border border-slate-200 rounded-lg px-3 py-1.5 text-xs text-slate-800 font-medium">
                                                    <?php $__currentLoopData = ['present', 'late', 'absent', 'on_leave', 'mc']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                        <option value="<?php echo e($s); ?>" <?php echo e($row->status === $s ? 'selected' : ''); ?>>
                                            ...
                                            <div class="flex-1 min-w-[200px]">
                                                ...
                                                <input type="text" name="remarks" value="<?php echo e($row->remarks); ?>"
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <button type="button"
                                                    onclick="document.getElementById('edit-<?php echo e($row->id); ?>').classList.add('hidden')"
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\laundrystaff-pro\resources\views/admin/attendance/index.blade.php ENDPATH**/ ?>